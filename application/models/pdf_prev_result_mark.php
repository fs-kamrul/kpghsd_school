...
class pdf_prev_result_mark extends CI_Model {
    public function __construct() {
        parent::__construct();
        $this->load->library('grade');
    }
    public function prev_select_result($data) {
        $datap = array();
        $datar = array();
        $datapass = array();
        $datapla = array();
        ...
        $grade = new Grade();
        foreach ($data['prev_reg'] as $key => $value) {
...
//        exit();
        $vn = $vn1 = $gpa = $gpa_one = $total_sub = $opsonl_sub = 0;
        $pf = $pfp = "";
...
//                    $vn1 +=$value2['total'];
                    ...
                    $datap[$value['id']]['prev_mark'][$key1] = $sum_t;
...
//                $datap[$value['id']]['cgpa'] = $gpa;
            ...
            $aa_cgpa= $datap[$value['id']]['cgpa']/($total_sub-$opsonl_sub);
            $aa_gpa= $datap[$value['id']]['gpa']/($total_sub-$opsonl_sub);
            if ($aa_cgpa>5) {
                $aa_cgpa=5;
            }
