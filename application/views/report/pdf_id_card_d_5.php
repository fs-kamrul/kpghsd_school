...
foreach ($main_a as $key => $value) {
    ...
    $pdf->Image('images/bg_02.jpg', $bgx, $bgy, $bgz,86);
    $pdf->Image('images/im_pdf/a40.png', $lx, $ly, $lz);
    $pdf->Image('images/border.jpg',$bx, $by, $bz,28.5);
...
//    $pdf->SetFont('SairaExtraCondensed', '', 13.5);
    ...
    $pdf->Cell($hef,3,"",'LTR',1,'R');
        $pdf->SetX($x);
    $pdf->Cell($hef,$th," ",'LR',1,'R');
        $pdf->SetX($x);
        $pdf->SetFont('Arial', 'B', 11);
    $pdf->Cell($hef,$th+2,"",'LR',1,'C');
        $pdf->SetX($x);
        $pdf->SetFillColor(215,40,45);
        $pdf->SetTextColor(0,0,160);
        $pdf->SetDrawColor(215,40,45);
    $pdf->SetFont('Arial', 'B', 11);
    $pdf->Cell($hef, 4, 'Collectorate Public College','LR', 1, 'C');
        $pdf->SetX($x);
        $pdf->SetFont('Arial', 'B', 11);
    $pdf->Cell($hef, 4, 'Nilphamari','LR', 1, 'C');
        $pdf->SetX($x);
    ...
    $pdf->Cell($hef, 4.5, '','LR',1,'R');
        ...
        $pdf->SetTextColor(0,0,0);
    $pdf->SetFont('Arial', 'B', 5);
    $pdf->Cell($hef, 2, 'Signature of the Principal   ','LR',1,'C');
        $pdf->SetX($x);
        $pdf->SetDrawColor(215,40,45);
    ...
    $pdf->Cell($hef, $th-3, '','',1,'C');
        $pdf->SetX($x);
    $pdf->SetFont('Arial', 'B', 10.5);
    $pdf->Cell($hef, 4, '','LBR',1,'C');
    ...
    if ($k==1) {
        ...
        $k=0;
        $y=$y+92;$x = 13.5;
        $px=34;$py+=92;$sy+=92;$sx=29.6;$lx=30;$ly+=92;$bx=28.5;$rx=52;$by+=92;$ry+=92;
        $bgx=13.5;$bgy=$y;
    }
}
$pdf->Output();
