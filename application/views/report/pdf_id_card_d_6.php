...
foreach ($main_a as $key => $value) {
    ...
    $pdf->Image('images/border_01.jpg',$bx, $by, $bz,30.2);
...
//    if(empty($picture_file)){
    ...
    $pdf->Image('images/bg_04.png', $bgx, $bgy, $bgz,86);
...
//    $pdf->SetFont('SairaExtraCondensed', '', 13.5);
    ...
    $pdf->Cell($hef,3,"",'LTR',1,'R');
        $pdf->SetX($x);
    $pdf->Cell($hef,$th-4,"Collectorate Public   ",'LR',1,'R');
        $pdf->SetX($x);
        $pdf->SetFont('SairaExtraCondensed', '', 11.5);
    $pdf->Cell($hef,$th+6,"School & College,Nilphamari",'LR',1,'R');
        $pdf->SetX($x);
    ...
    $pdf->Cell($hef,$th-1,"         Section: $value->section",'LR',1,'L');
        $pdf->SetX($x);
    $pdf->SetFont('Arial', 'B', 9);
    $pdf->Cell($hef,$th-1,"         Session: 2019",'LR',1,'L');
        $pdf->SetX($x);
    $pdf->SetFont('Arial', 'B', 9);
    ...
    $pdf->Cell($hef, $th-3+2, 'www.cpscn.edu.bd','LBR',1,'C');
    ...
    if ($k==1) {
        ...
        $k=0;
        $y=$y+92;$x = 13.5;
        $px=14;$py+=92;$sy+=92;$sx=28;$lx=46;$ly+=92;$bx=24;$rx=54;$by+=92;$ry+=92;
        $bgx=13.5;$bgy=$y;
    }
}
$pdf->Output();
