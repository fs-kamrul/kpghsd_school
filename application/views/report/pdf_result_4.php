...
//        }
    ...
    $i = $grade->num_j($j / 2);
    $j = 0;$sum2=0;$grou=0;$grou_g1=0;$grou1=0;$tota_mark=0;    //$grade->cut_plus($sum_t);
...
//        if ($v_sub->add_f == null || $v_sub->add_f == 1 || $v_sub->add_f == 2) {
            ...
            if ($value[$value1['sub_name']]['add_fail'] == 1) {
                ...
                $sum_main_mark_sum=$value[$value1['sub_name']]['sum_main_mark_sum'];

            $sum2+=$value[$value1['sub_name']]['tota_mark'];
            $tota_mark += $value[$value1['sub_name']]['sum_main_mark_sum'];
//            $pdf->Cell(39, $th, '( ' . $grade->cut_plus_1($value[$value1['sub_name']]['to_mark'], 2) . ' )', 1, 0, 'C');
            $pdf->Cell(39, $th, '( ' . $grade->cut_plus_1($value[$value1['sub_name']]['main_mark'], 2) . ' )', 1, 0, 'C');
            if($value1['add_group']==0){
//                $grou=0;
                $pdf->Cell(25, $th, round($sum_main_mark_sum, 2), 1, 0, 'C');
            }elseif($value1['add_group']==1){
//                if ($value[$value1['sub_name']]['add_fail'] == 1) {
                    $grou+=$sum1;
                    $grou_g1+=$sum_main_mark_sum;
//                }  else {
//                    $grou=0;
//            }
                ...
                $grou2=$grou/2;
                $grou2_g1=$grou_g1/2;
                $pdf->Cell(25, $th-7, substr(round($grou2_g1, 2),0,5), "FLR", 0, 'C');
                $grou=0;
            }

            $pdf->Cell(28, $th, round($sub_info[$value1['sub_name']]['high_mark_g1'], 2), 1, 0, 'C');
...
//    $pdf->Cell(25, $th, $sum2, 0, 0, 'C');
    $pdf->Cell(25, $th, $tota_mark, 0, 0, 'C');
